...
internal sealed class GetOrderQueryHandler(IOrderRepository repository, IProductRepository repositoryProduct) : IQueryHandler<GetOrderQuery, IEnumerable<OrderDto>>
{
    public async Task<Result<IEnumerable<OrderDto>>> Handle(GetOrderQuery request, CancellationToken cancellationToken)
    {
        var orders = await repository.GetAllAsync().ConfigureAwait(false);
